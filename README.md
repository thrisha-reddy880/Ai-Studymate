...
Render will create the ai-studymate web service and ai-studymate-db PostgreSQL database from render.yaml.
In the web service's Environment settings, add OPENAI_API_KEY with your secret API key. JWT_SECRET is generated automatically by the Blueprint and DATABASE_URL is connected to the Postgres database.
Deploy. Render will run npm install and npm start and expose the app at a public onrender.com URL.
...
The online version uses PostgreSQL when DATABASE_URL is present.
The local JSON file remains as a fallback for local development.
AI Tutor
