...
$("#nextCard").onclick=()=>{if(!state.flashList.length)return;state.flashIndex=(state.flashIndex+1)%state.flashList.length;renderFlashcard()};
$("#shuffleCards").onclick=()=>{state.flashList.sort(()=>Math.random()-.5);state.flashIndex=0;renderFlashcard()};
$("#flashSubjectFilter").onchange=()=>{state.flashIndex=0;initFlashcards()};
$("#chatForm").onsubmit=e=>{e.preventDefault();sendChat()};
$("#quickAsk").onclick=()=>{navigate("tutor");$("#chatInput").focus()};
$("#insightBtn").onclick=generateInsight;
$("#dashTimerBtn").onclick=toggleTimer;

async function boot() {
  if(!state.token){showAuth();return}
  try { const me=await api("/api/me");state.user=me.user;showApp();await loadData(); } catch { showAuth(); }
}
boot();
